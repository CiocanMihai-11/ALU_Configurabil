...
`ifndef __req_ack_coverage_collector
`define __req_ack_coverage_collector
...
class coverage_req_ack extends uvm_component;
  
  //componenta se adauga in baza de date
  `uvm_component_utils(coverage_req_ack)
  
  //se declara pointerul catre monitorul care da datele asupra carora se vor face masuratorile de coverage
  monitor_req_ack p_monitor;
  
  covergroup stari_req_ack_cg;
    option.per_instance = 1;
    coverpoint p_monitor.starea_preluata_a_req_ack.addr{
        bins min_addr = {0};
        bins random_addr = {[1:3]};
      	bins max_addr = {4};
    }
    coverpoint p_monitor.starea_preluata_a_req_ack.irq;
  endgroup
